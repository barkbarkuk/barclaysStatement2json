...
var BSParserUtils = (function () {
    ...
    var parseStatementPeriodText = function (text) {
    	...
    	if (match = /(?:(\d+)(?:\s+([A-Za-z]+)(?:\s+(\d+))?)?).+?(\d+\s+([A-Za-z]+)\s+(\d+))/.exec(statementPeriod)) {
    		var start = new Date((match[3] ? match[3] : match[6]) + ' ' + (match[2] ? match[2] : match[5]) + ' ' + match[1]);
    		var end = new Date(match[4]);
